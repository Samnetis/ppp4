...
#data_path = r'C:\Users\wealt\Documents\machine learning\Car_dekho_cleaned_dataset.csv',
...
result_background_color = "#FFF8E7"
...
# Ensure the 'mileage' column is numeric
filtered_data['mileage'] = pd.to_numeric(filtered_data['mileage'], errors='coerce')
# Drop any NaN values in 'mileage' column before calculating min and max
filtered_data = filtered_data.dropna(subset=['mileage'])

# Calculate min and max mileage, rounding appropriately
min_mileage = np.floor(filtered_data['mileage'].min()) if not filtered_data['mileage'].empty else 0
max_mileage = np.ceil(filtered_data['mileage'].max()) if not filtered_data['mileage'].empty else 100

# Mileage slider in sidebar with a unique key
mileage = st.sidebar.slider('11. Mileage (kmpl)', 
                            min_value=float(min_mileage), 
                            max_value=float(max_mileage), 
                            value=float(min_mileage), 
                            step=0.5, 
                            key="mileage_slider_unique")

# Ensure 'Seats' column is numeric (apply similar steps for other numeric columns like km)
filtered_data['Seats'] = pd.to_numeric(filtered_data['Seats'], errors='coerce')
filtered_data = filtered_data.dropna(subset=['Seats'])

# Ensure 'City' column exists in the data
if 'City' in data.columns:
    city = visual_selectbox('12. City', sorted(data['City'].unique()))
else:
    st.sidebar.error('City column not found in the dataset.')
...
user_input_data = {
    ...
    'City': [city],  # Add the city variable here
    'mileage': [mileage],
    'Seats': [seat_count],
